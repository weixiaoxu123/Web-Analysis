...
def feature_vector(stri):
	...
	if stri=="benign":
	 	data = json.load(open("alexa.json", 'r'))
		...
		while i <len(final_list):
			...
			while j <len(final_list):
				if final_list[i][7]==final_list[j][7]:
					count+=1;
				j+=1
			k=0
			while k <len(final_list):
				if final_list[i][7]==final_list[k][7]:
					final_list[k][7]=count;
				k+=1
			i+=1
		print final_list
		print len(final_list)
		with open("f_vectorsmultinb.csv", "a") as f:
			writer = csv.writer(f)
			writer.writerows(final_list)
		return final_list

	elif stri=="malicious":
		data = json.load(open("allmalicious.json", 'r'))
		ec=0
		pc=0
		...
		print len(final_list)
		with open("f_vectors2multinb.csv", "a") as f:
			writer = csv.writer(f)
			writer.writerows(final_list)
		return final_list
